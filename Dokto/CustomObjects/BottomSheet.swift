...
open class BottomSheet {
    open class Controller: UIViewController {
        ...
        open func addSearchBar(configurationHandler: ((UISearchBar) -> Void)) {
            let searchbar = UISearchBar.init(frame: CGRect.init(x: 0, y: 44, width: view.frame.width, height: 44))
            containerView.addSubview(searchbar)
            configurationHandler(searchbar)
        }
